...
#define ASSERT(_CONDITION_) \
	((_CONDITION_) ? DMG_STATUS_SUCCESS : DMG_STATUS_FAILURE)

#define ASSERT_FAILURE(_CONDITION_) \
	((_CONDITION_ == DMG_STATUS_FAILURE) ? DMG_STATUS_SUCCESS : DMG_STATUS_FAILURE)

#define ASSERT_SUCCESS(_CONDITION_) \
	((_CONDITION_ == DMG_STATUS_SUCCESS) ? DMG_STATUS_SUCCESS : DMG_STATUS_FAILURE)
...
#define TRACE_TEST(_RESULT_) { \
		TRACE_ENABLE(NULL); \
		if(_RESULT_ != DMG_STATUS_SUCCESS) { \
