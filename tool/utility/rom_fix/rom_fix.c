	...
	)
{
	...
	if(verbose) {
		dmg_utility_rom_fix_version(stream, verbose);
		TRACE_TOOL(stream, LEVEL_NONE, "%s", "\n");
	}
	...
	)
{
	...
	if(g_rom_fix.help) {
		dmg_utility_rom_fix_usage(stdout, true);
	} else if(g_rom_fix.version) {
		dmg_utility_rom_fix_version(stdout, false);
	} else {
