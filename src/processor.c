...
static dmg_error_t dmg_processor_instruction_adc(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        ...
        case 0x8E: /* (HL) */
            handle->processor.delay += 4;
            operand = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x8F: /* A */
        ...
        case 0xCE: /* # */
            handle->processor.delay += 4;
            operand = dmg_bus_read(handle, handle->processor.pc.word++);
            break;
    }
...
{
    uint32_t carry, sum;
    int8_t operand = dmg_bus_read(handle, handle->processor.pc.word++);
...
static dmg_error_t dmg_processor_instruction_and(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        ...
        case 0xA6: /* (HL) */
            handle->processor.delay += 4;
            handle->processor.af.high &= dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0xA7: /* A */
            break;
        case 0xE6: /* # */
            handle->processor.delay += 4;
            handle->processor.af.high &= dmg_bus_read(handle, handle->processor.pc.word++);
            break;
    }
...
static dmg_error_t dmg_processor_instruction_bit(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        ...
        case 0x66: case 0x6E: case 0x76: case 0x7E: /* (HL) */
            handle->processor.delay += 4;
            handle->processor.af.zero = !(dmg_bus_read(handle, handle->processor.hl.word) & (1 << ((handle->processor.instruction.opcode - 0x46) / 8)));
            break;
...
static dmg_error_t dmg_processor_instruction_call(dmg_handle_t const handle)
{
    ...
    operand.low = dmg_bus_read(handle, handle->processor.pc.word++);
    operand.high = dmg_bus_read(handle, handle->processor.pc.word++);
    switch (handle->processor.instruction.opcode)
    {
        case 0xC4: /* NZ */
    ...
    {
        handle->processor.delay += 12;
        dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.pc.high);
        dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.pc.low);
...
static dmg_error_t dmg_processor_instruction_dec(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        ...
        case 0x35: /* (HL) */
            handle->processor.delay += 8;
            operand = dmg_bus_read(handle, handle->processor.hl.word) - 1;
            dmg_bus_write(handle, handle->processor.hl.word, operand);
            break;
        case 0x3D: /* A */
            operand = --handle->processor.af.high;
            break;
...
static dmg_error_t dmg_processor_instruction_ld(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        case 0x01: /* BC,## */
            handle->processor.delay += 8;
            handle->processor.bc.low = dmg_bus_read(handle, handle->processor.pc.word++);
            handle->processor.bc.high = dmg_bus_read(handle, handle->processor.pc.word++);
            break;
        case 0x02: /* (BC),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.bc.word, handle->processor.af.high);
            break;
        case 0x06: /* B,# */
            handle->processor.delay += 4;
            handle->processor.bc.high = dmg_bus_read(handle, handle->processor.pc.word++);
        ...
        case 0x08: /* (##),SP */
            ...
            operand.low = dmg_bus_read(handle, handle->processor.pc.word++);
            operand.high = dmg_bus_read(handle, handle->processor.pc.word++);
            dmg_bus_write(handle, operand.word, handle->processor.sp.low);
            dmg_bus_write(handle, operand.word + 1, handle->processor.sp.high);
            break;
        case 0x0A: /* A,(BC) */
            handle->processor.delay += 4;
            handle->processor.af.high = dmg_bus_read(handle, handle->processor.bc.word);
            break;
        case 0x0E: /* C,# */
            handle->processor.delay += 4;
            handle->processor.bc.low = dmg_bus_read(handle, handle->processor.pc.word++);
        ...
        case 0x11: /* DE,## */
            handle->processor.delay += 8;
            handle->processor.de.low = dmg_bus_read(handle, handle->processor.pc.word++);
            handle->processor.de.high = dmg_bus_read(handle, handle->processor.pc.word++);
            break;
        case 0x12: /* (DE),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.de.word, handle->processor.af.high);
            break;
        case 0x16: /* D,# */
            handle->processor.delay += 4;
            handle->processor.de.high = dmg_bus_read(handle, handle->processor.pc.word++);
        ...
        case 0x1A: /* A,(DE) */
            handle->processor.delay += 4;
            handle->processor.af.high = dmg_bus_read(handle, handle->processor.de.word);
            break;
        case 0x1E: /* E,# */
            handle->processor.delay += 4;
            handle->processor.de.low = dmg_bus_read(handle, handle->processor.pc.word++);
        ...
        case 0x21: /* HL,## */
            handle->processor.delay += 8;
            handle->processor.hl.low = dmg_bus_read(handle, handle->processor.pc.word++);
            handle->processor.hl.high = dmg_bus_read(handle, handle->processor.pc.word++);
            break;
        case 0x22: /* (HL+),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word++, handle->processor.af.high);
            break;
        case 0x26: /* H,# */
            handle->processor.delay += 4;
            handle->processor.hl.high = dmg_bus_read(handle, handle->processor.pc.word++);
        ...
        case 0x31: /* SP,## */
            handle->processor.delay += 8;
            handle->processor.sp.low = dmg_bus_read(handle, handle->processor.pc.word++);
            handle->processor.sp.high = dmg_bus_read(handle, handle->processor.pc.word++);
            break;
        case 0x32: /* (HL-),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word--, handle->processor.af.high);
        ...
        case 0x36: /* (HL),# */
            handle->processor.delay += 8;
            dmg_bus_write(handle, handle->processor.hl.word, dmg_bus_read(handle, handle->processor.pc.word++));
            break;
        case 0x3A: /* A,(HL-) */
            handle->processor.delay += 4;
            handle->processor.af.high = dmg_bus_read(handle, handle->processor.hl.word--);
        ...
        case 0x46: /* B,(HL) */
            handle->processor.delay += 4;
            handle->processor.bc.high = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x47: /* B,A */
        ...
        case 0x4E: /* C,(HL) */
            handle->processor.delay += 4;
            handle->processor.bc.low = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x4F: /* C,A */
        ...
        case 0x56: /* D,(HL) */
            handle->processor.delay += 4;
            handle->processor.de.high = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x57: /* D,A */
        ...
        case 0x5E: /* E,(HL) */
            handle->processor.delay += 4;
            handle->processor.de.low = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x5F: /* E,A */
        ...
        case 0x66: /* H,(HL) */
            handle->processor.delay += 4;
            handle->processor.hl.high = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x67: /* H,A */
        ...
        case 0x6E: /* L,(HL) */
            handle->processor.delay += 4;
            handle->processor.hl.low = dmg_bus_read(handle, handle->processor.hl.word);
            break;
        case 0x6F: /* L,A */
        ...
        case 0x70: /* (HL),B */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.bc.high);
            break;
        case 0x71: /* (HL),C */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.bc.low);
            break;
        case 0x72: /* (HL),D */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.de.high);
            break;
        case 0x73: /* (HL),E */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.de.low);
            break;
        case 0x74: /* (HL),H */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.hl.high);
            break;
        case 0x75: /* (HL),L */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.hl.low);
            break;
        case 0x77: /* (HL),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, handle->processor.hl.word, handle->processor.af.high);
        ...
        case 0xE0: /* (FF00+#),A */
            handle->processor.delay += 8;
            dmg_bus_write(handle, 0xFF00 + dmg_bus_read(handle, handle->processor.pc.word++), handle->processor.af.high);
            break;
        case 0xE2: /* (FF00+C),A */
            handle->processor.delay += 4;
            dmg_bus_write(handle, 0xFF00 + handle->processor.bc.low, handle->processor.af.high);
            break;
        case 0xEA: /* (##),A */
            handle->processor.delay += 12;
            operand.low = dmg_bus_read(handle, handle->processor.pc.word++);
            operand.high = dmg_bus_read(handle, handle->processor.pc.word++);
            dmg_bus_write(handle, operand.word, handle->processor.af.high);
            break;
        case 0xF0: /* A,(FF00+#) */
            handle->processor.delay += 8;
            handle->processor.af.high = dmg_bus_read(handle, 0xFF00 + dmg_bus_read(handle, handle->processor.pc.word++));
            break;
        case 0xF2: /* A,(FF00+C) */
            handle->processor.delay += 4;
            handle->processor.af.high = dmg_bus_read(handle, 0xFF00 + handle->processor.bc.low);
            break;
        case 0xF9: /* SP,HL */
            handle->processor.delay += 4;
        ...
        case 0xFA: /* A,(##) */
            ...
            operand.low = dmg_bus_read(handle, handle->processor.pc.word++);
            operand.high = dmg_bus_read(handle, handle->processor.pc.word++);
            handle->processor.af.high = dmg_bus_read(handle, operand.word);
            break;
    }
    return DMG_SUCCESS;
}
...
static dmg_error_t dmg_processor_instruction_pop(dmg_handle_t const handle)
{
    ...
    {
        case 0xC1: /* BC */
            handle->processor.bc.low = dmg_bus_read(handle, handle->processor.sp.word++);
            handle->processor.bc.high = dmg_bus_read(handle, handle->processor.sp.word++);
            break;
        case 0xD1: /* DE */
            handle->processor.de.low = dmg_bus_read(handle, handle->processor.sp.word++);
            handle->processor.de.high = dmg_bus_read(handle, handle->processor.sp.word++);
            break;
        case 0xE1: /* HL */
            handle->processor.hl.low = dmg_bus_read(handle, handle->processor.sp.word++);
            handle->processor.hl.high = dmg_bus_read(handle, handle->processor.sp.word++);
            break;
        case 0xF1: /* AF */
            handle->processor.af.low = dmg_bus_read(handle, handle->processor.sp.word++) & 0xF0;
            handle->processor.af.high = dmg_bus_read(handle, handle->processor.sp.word++);
            break;
    }
    return DMG_SUCCESS;
}
...
static dmg_error_t dmg_processor_instruction_push(dmg_handle_t const handle)
{
    ...
    {
        case 0xC5: /* BC */
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.bc.high);
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.bc.low);
            break;
        case 0xD5: /* DE */
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.de.high);
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.de.low);
            break;
        case 0xE5: /* HL */
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.hl.high);
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.hl.low);
            break;
        case 0xF5: /* AF */
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.af.high);
            dmg_bus_write(handle, --handle->processor.sp.word, handle->processor.af.low);
            break;
    }
    return DMG_SUCCESS;
}
...
static dmg_error_t dmg_processor_instruction_res(dmg_handle_t const handle)
{
    ...
    switch (handle->processor.instruction.opcode)
    {
        ...
        case 0xA6: case 0xAE: case 0xB6: case 0xBE: /* (HL) */
            handle->processor.delay += 8;
            dmg_bus_write(handle, handle->processor.hl.word, dmg_bus_read(handle, handle->processor.hl.word)
...
static dmg_error_t dmg_processor_instruction_ret(dmg_handle_t const handle)
{
    ...
    if (taken)
    {
        ...
        handle->processor.pc.low = dmg_bus_read(handle, handle->processor.sp.word++);
        handle->processor.pc.high = dmg_bus_read(handle, handle->processor.sp.word++);
    }
    return DMG_SUCCESS;
}
...
static dmg_error_t dmg_processor_instruction_stop(dmg_handle_t const handle)
{
    ...
    dmg_bus_read(handle, handle->processor.pc.word++);
    return DMG_SUCCESS;
}
...
static dmg_error_t dmg_processor_execute(dmg_handle_t const handle)
{
    ...
    handle->processor.instruction.opcode = dmg_bus_read(handle, handle->processor.pc.word++);
